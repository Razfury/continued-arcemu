...
public:
	...
	void OnPlayerEnter(Player* pPlayer)
	{
		...
		Creature *spawner = pPlayer->GetMapMgr()->GetInterface()->GetCreatureNearestCoords(pPlayer->GetPositionX(), pPlayer->GetPositionY(), pPlayer->GetPositionZ(), 41839);
...
class SinclariAI : public AICreatureScript
{
public:

	AI_CREATURE_SCRIPT_FUNCTION(SinclariAI, AICreatureScript);
	SinclariAI(Creature* pCreature) : AICreatureScript(pCreature)
	{
...
public:
	...
	void GossipHello(Object* pObject, Player* pPlayer)
	{
		...
		if (pInstance->GetInstanceData(Data_EncounterState, MAP_VIOLET_HOLD) == State_NotStarted)
		{
			...
			menu->AddItem(0, "Activate the crystals when we get in trouble, right.", 1);
		...
		if (pInstance->GetInstanceData(Data_EncounterState, MAP_VIOLET_HOLD) == State_InProgress)
		{
			...
			menu->AddItem(0, "Send me back in.", 3);
	...
	void GossipSelectOption(Object* pObject, Player*  pPlayer, uint32 Id, uint32 IntId, const char* Code)
	{
		...
		case 1:
		{
			...
			menu->AddItem(0, "Get your people to safety, we will hold back the blue dragonflight.", 2);
			menu->SendTo(pPlayer);

			// New Encounter State included
			pInstance->SetInstanceData(Data_EncounterState, MAP_VIOLET_HOLD, 4);
		}break;

		case 2:
		{
			pPlayer->Gossip_Complete();
			pCreature->GetAIInterface()->MoveTo(1813.31f, 804.113f, 44.364f, -1.31f);
			TO_CREATURE(pObject)->GetMapMgr()->GetInterface()->SpawnCreature(41839, 1883.76f, 804.73f, 38.39f, 3.2f, true, false, 0, 0);
			TO_CREATURE(pObject)->SendChatMessage(CHAT_MSG_MONSTER_SAY, LANG_UNIVERSAL, SINCLARY_SAY_2);
			TO_CREATURE(pObject)->SetUInt32Value(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_GOSSIP);
			TheVioletHoldScript* pInstance = (TheVioletHoldScript*)TO_CREATURE(pObject)->GetMapMgr()->GetScript();
			if (pInstance)
			{
				pInstance->SetInstanceData(Data_EncounterState, MAP_VIOLET_HOLD, State_InProgress);
				pInstance->SetData(PORTAL_NUMBER, 1);
		...
		case 3:
		{
			...
			pPlayer->SafeTeleport(MAP_VIOLET_HOLD, pPlayer->GetInstanceID(), 1830.531006f, 803.939758f, 44.340508f, 6.281611f);
		}break;
		}
	}
};
...
///////////////////////////////////////////////////////
//VH Guards
class VHGuardsAI : public AICreatureScript
{
public:

	AI_CREATURE_SCRIPT_FUNCTION(VHGuardsAI, AICreatureScript);
	VHGuardsAI(Creature* pCreature) : AICreatureScript(pCreature)
	{
		_unit->GetAIInterface()->setMoveType(MOVEMENTTYPE_WANTEDWP);
	}
...
class VioletHoldSpawnHandler : public AICreatureScript
{
public:
	AI_CREATURE_SCRIPT_FUNCTION(VioletHoldSpawnHandler, AICreatureScript);
	VioletHoldSpawnHandler(Creature* pCreature) : AICreatureScript(pCreature)
	{
		RegisterAIUpdateEvent(1000);
	}
	...
	void OnLoad()
	{
		if (_unit->GetMapMgr()->GetMapId() == MAP_VIOLET_HOLD)
		{
			_phase = 0;
			RegisterAIUpdateEvent(1000);
			SetPhase(1);
	...
	void UpdateAI()
	{
		...
		if (IsTimerFinished(wavetimer) && _phase == 2) // 1/18
		{
			switch (urand(0, 2))
			{
			case 0:
		...
		if (IsTimerFinished(wavetimer) && _phase == 3) // 2/18
		{
		...
		if (IsTimerFinished(wavetimer) && _phase == 4) // 3/18
		{
		...
		if (IsTimerFinished(wavetimer) && _phase == 5) // 4/18
		{
		...
		if (IsTimerFinished(wavetimer) && _phase == 6) // 5/18
		{
		...
		if (IsTimerFinished(wavetimer) && _phase == 7) // 6/18 first boss
		{
		...
		if (IsTimerFinished(wavetimer) && _phase == 8) // 7/18
		{
		...
		if (IsTimerFinished(wavetimer) && _phase == 9) // 8/18
		{
		...
		if (IsTimerFinished(wavetimer) && _phase == 10) // 9/18
		{
		...
		if (IsTimerFinished(wavetimer) && _phase == 11) // 10/18
		{
		...
		if (IsTimerFinished(wavetimer) && _phase == 12) // 11/18
		{
		...
		if (IsTimerFinished(wavetimer) && _phase == 13) // 12/18 second boss
		{
		...
		if (IsTimerFinished(wavetimer) && _phase == 14) // 13/18
		{
		...
		if (IsTimerFinished(wavetimer) && _phase == 15) // 14/18
		{
		...
		if (IsTimerFinished(wavetimer) && _phase == 16) // 15/18
		{
		...
		if (IsTimerFinished(wavetimer) && _phase == 17) // 16/18
		{
		...
		if (IsTimerFinished(wavetimer) && _phase == 18) // 17/18
		{
		...
		if (IsTimerFinished(wavetimer) && _phase == 19)
		{
...
class AzureBinderAI : public AICreatureScript
{
public:
	AI_CREATURE_SCRIPT_FUNCTION(AzureBinderAI, AICreatureScript);
	AzureBinderAI(Creature* pCreature) : AICreatureScript(pCreature)
	{
	}
	...
	void UpdateAI()
	{
		...
		Creature* erekem_guard_1 = _unit->GetMapMgr()->GetInterface()->GetCreatureSpawnIdNearestCoords(1892.42f, 872.28f, 43.41f, 29395, 6080021);
		Creature* erekem_guard_2 = _unit->GetMapMgr()->GetInterface()->GetCreatureSpawnIdNearestCoords(1853.75f, 862.45f, 43.41f, 29395, 6080020);
		...
		if (IsTimerFinished(Move_Timer) && _binderPhase == 0)
		{
			RandomBossSelection();
			SetPhase(1);
		}

		if (IsTimerFinished(ReleaseTimer_Erekem) && _binderPhase == 1)
		{
			if (erekem)
			{
				if (erekem_guard_1)
				{
					erekem_guard_1->removeUnitFlags();
					erekem_guard_1->GetAIInterface()->MoveTo(1881.57f, 852.61f, 43.33f, 4.7f);
				}
				if (erekem_guard_2)
				{
					erekem_guard_2->removeUnitFlags();
		...
		if (IsTimerFinished(ReleaseTimer_Moragg) && _binderPhase == 1)
		{
			if (moragg)
			{
				if (moragg_go)
		...
		if (IsTimerFinished(ReleaseTimer_Ichoron) && _binderPhase == 1)
		{
			if (ichron)
			{
				if (ichron_go)
		...
		if (IsTimerFinished(ReleaseTimer_Xevozz) && _binderPhase == 1)
		{
			if (xevozz)
			{
				if (xevozz_go)
		...
		if (IsTimerFinished(ReleaseTimer_Lavanthos) && _binderPhase == 1)
		{
			if (lavanthos)
			{
				if (lavanthos_go)
		...
		if (IsTimerFinished(ReleaseTimer_Zuramat) && _binderPhase == 1)
		{
			if (zuramat)
			{
				if (zuramat_go)
...
class POrtalAI : public AICreatureScript
{
public:
	AI_CREATURE_SCRIPT_FUNCTION(POrtalAI, AICreatureScript);
	POrtalAI(Creature* pCreature) : AICreatureScript(pCreature)
	{
	}
	...
	void OnLoad()
	{
		...
		RegisterAIUpdateEvent(urand(6000, 9000));
		Despawn(26000, 0);
		ParentClass::OnLoad();
	}
...
class ErekemAI : public AICreatureScript
{
public:
	AI_CREATURE_SCRIPT_FUNCTION(ErekemAI, AICreatureScript);
	ErekemAI(Creature* pCreature) : AICreatureScript(pCreature)
	{
		_addDeath = 0;
	}
	...
	uint64 GetChainHealTargetGUID()
	{
		...
		Creature *pGuard1 = _unit->GetMapMgr()->GetInterface()->GetCreatureSpawnIdNearestCoords(_unit->GetPositionX(), _unit->GetPositionY(), _unit->GetPositionZ(), 29395, 6080020);
		Creature *pGuard2 = _unit->GetMapMgr()->GetInterface()->GetCreatureSpawnIdNearestCoords(_unit->GetPositionX(), _unit->GetPositionY(), _unit->GetPositionZ(), 29395, 6080021);

		if (Creature *c1 = _unit->GetMapMgr()->GetInterface()->GetCreatureSpawnIdNearestCoords(_unit->GetPositionX(), _unit->GetPositionY(), _unit->GetPositionZ(), 29395, 6080020))
			if (c1->isAlive() && !c1->GetHealthPct() >= 75)
				return c1->GetGUID();

		if (Creature *c2 = _unit->GetMapMgr()->GetInterface()->GetCreatureSpawnIdNearestCoords(_unit->GetPositionX(), _unit->GetPositionY(), _unit->GetPositionZ(), 29395, 6080021))
...
class MoraggAI : public AICreatureScript
{
public:
	AI_CREATURE_SCRIPT_FUNCTION(MoraggAI, AICreatureScript);
	MoraggAI(Creature* pCreature) : AICreatureScript(pCreature)
	{
	}
...
class ErekemGuardAI : public AICreatureScript
{
public:
	AI_CREATURE_SCRIPT_FUNCTION(ErekemGuardAI, AICreatureScript);
	ErekemGuardAI(Creature* pCreature) : AICreatureScript(pCreature)
	{
	}
...
class IchoronAI : public AICreatureScript
{
public:
	AI_CREATURE_SCRIPT_FUNCTION(IchoronAI, AICreatureScript);
	IchoronAI(Creature* pCreature) : AICreatureScript(pCreature)
	{
	}
};
...
class XevozzAI : public AICreatureScript
{
public:
	AI_CREATURE_SCRIPT_FUNCTION(XevozzAI, AICreatureScript);
	XevozzAI(Creature* pCreature) : AICreatureScript(pCreature)
	{
	}
...
class EtherealSphereAI : public AICreatureScript
{
public:
	AI_CREATURE_SCRIPT_FUNCTION(EtherealSphereAI, AICreatureScript);
	EtherealSphereAI(Creature* pCreature) : AICreatureScript(pCreature)
	{
	}
...
class LavanthosAI : public AICreatureScript
{
public:
	AI_CREATURE_SCRIPT_FUNCTION(LavanthosAI, AICreatureScript);
	LavanthosAI(Creature* pCreature) : AICreatureScript(pCreature)
	{}
	...
	void EnterCombat(Unit* pAttacker)
	{
		...
		ParentClass::EnterCombat(pAttacker);
	}
...
class ZuramatTheObliteratorAI : public AICreatureScript
{
public:
	AI_CREATURE_SCRIPT_FUNCTION(ZuramatTheObliteratorAI, AICreatureScript);
	ZuramatTheObliteratorAI(Creature* pCreature) : AICreatureScript(pCreature)
	{
...
class PrisonSealDoorAI : public AICreatureScript // Npc Id: 30896
{
public:
	AI_CREATURE_SCRIPT_FUNCTION(PrisonSealDoorAI, AICreatureScript);
	PrisonSealDoorAI(Creature* pCreature) : AICreatureScript(pCreature)
	{
	}
...
class AzureCaptainAI : public AICreatureScript
{
public:
	AI_CREATURE_SCRIPT_FUNCTION(AzureCaptainAI, AICreatureScript);
	AzureCaptainAI(Creature* pCreature) : AICreatureScript(pCreature)
	{
	...
	void UpdateAI()
	{
		...
		if (IsTimerFinished(attackTimer))
		{
			if (PrisonSeal)
			{
				_unit->GetAIInterface()->MoveTo(PrisonSeal->GetPositionX(), PrisonSeal->GetPositionY(), PrisonSeal->GetPositionZ(), PrisonSeal->GetOrientation());
...
class AzureRaiderAI : public AICreatureScript
{
public:
	AI_CREATURE_SCRIPT_FUNCTION(AzureRaiderAI, AICreatureScript);
	AzureRaiderAI(Creature* pCreature) : AICreatureScript(pCreature)
	{
		attackTimer = AddTimer(2000);
	}
...
class AzureStalkerAI : public AICreatureScript
{
public:
	AI_CREATURE_SCRIPT_FUNCTION(AzureStalkerAI, AICreatureScript);
	AzureStalkerAI(Creature* pCreature) : AICreatureScript(pCreature)
	{
		attackTimer = AddTimer(2000);
	}
...
class CyanigosaAI : public AICreatureScript
{
public:
	AI_CREATURE_SCRIPT_FUNCTION(CyanigosaAI, AICreatureScript);
	CyanigosaAI(Creature* pCreature) : AICreatureScript(pCreature)
	{
		timerjump = 0;
		attackable = 0;
	...
	void OnLoad()
	{
		...
		_unit->SetUInt32Value(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_ATTACKABLE_2); //Unattackable
	...
	void UpdateAI()
	{
		if (IsTimerFinished(timerjump) && _cyanigosaPhase == 10)
		{
			_unit->GetAIInterface()->MoveJump(1892.40f, 804.39f, 38.42f, 3.1f);
			attackable = AddTimer(6000);
			SetPhase(11);
		}
		if (IsTimerFinished(attackable) && _cyanigosaPhase == 11)
		{
...
void SetupVioletHold(ScriptMgr* mgr)
{
	...
	//Bosses
	mgr->register_creature_script(41839, &VioletHoldSpawnHandler::Create);
